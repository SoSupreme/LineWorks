    ...
    addEvent(goodsSection){ 
        goodsSection.addEventListener('click', function(event){
            if(event.target.className == "product" || event.target.className ==="price"){
              ...
              let check = BuyProducts.checkLeft(event.target.parentElement.firstElementChild.innerHTML);
...
BuyProducts.checkLeft = function(value){ // 재고가 남아 있으면 해당 제품 idx 아니면 -1
  let idx;
